...
> **Full Documentation**: See `.agent/rules/srpi.md`

This is the standard workflow for all new features. Follow the **Spec-Driven Development (SDD)** three-phase workflow. Each phase requires explicit human approval before proceeding.

**Overview**:

1. **Spec (S)**: Define requirements and acceptance criteria.
2. **Research (R)**: Investigate technology and best practices.
3. **Plan (P)**: Create detailed step-by-step execution tasks.
4. **Implement (I)**: Execute the plan.

See detailed rules in `.agent/rules/srpi.md`.
